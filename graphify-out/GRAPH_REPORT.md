# Graph Report - pythons  (2026-07-21)

## Corpus Check
- 145 files · ~21,873 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 727 nodes · 529 edges · 252 communities (227 shown, 25 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 13 edges (avg confidence: 0.66)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `84f54d54`
...
Cohesion: 0.06
Nodes (21): 应用入口：路由装饰器、中间件、请求分发。  调用链（自外向内）：      socket 收到字节       → Request.from_raw, 阻塞启动内置 socket 服务器（Ctrl+C 结束）。, 纯 Python 原生实现的迷你 HTTP 框架（仅依赖标准库）。  对外只暴露应用入口与请求/响应类型，内部模块拆分见同目录各文件。, HTTP 请求对象：解析请求行、请求头、查询参数与请求体。  一次完整请求在 TCP 字节流中的大致形态：      GET /api/hello?name=P, 封装一次 HTTP 请求的解析结果，供路由处理函数使用。, 取某个查询参数的第一个值（多值时常用这种简化接口）。, 解析 application/x-www-form-urlencoded 表单。, 将请求体解析为 JSON（application/json）。 (+13 more)
...
## Knowledge Gaps
- **189 isolated node(s):** `Config`, `计算圆的面积     :param radius: 圆的半径     :return: 圆的面积`, `统计文本中字母的个数和字母的组合     :param text: 文本     :return: 字母的个数和字母的组合`, `统计文本中元音字母的个数     :param text: 文本     :return: 元音字母的个数`, `统计分数列表中最高分 最低分 和平均分     :param scores: 分数列表     :return: 最高分 最低分 和平均分` (+184 more)
...
- **Why does `MiniHTTP` connect `Community 2` to `Community 1`, `Community 3`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `MethodNotAllowed` connect `Community 3` to `Community 2`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `Response` connect `Community 2` to `Community 1`?**
...
- **What connects `Config`, `计算圆的面积     :param radius: 圆的半径     :return: 圆的面积`, `统计文本中字母的个数和字母的组合     :param text: 文本     :return: 字母的个数和字母的组合` to the rest of the system?**
